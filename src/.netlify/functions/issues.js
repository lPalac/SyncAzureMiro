...
exports.handler = async function (event) {
  ...
  const azurePBIId = resource.id;
  const azurePBINumber = resource.fields["System.Id"];
  const title = resource.fields["System.Title"];
  const description = resource.fields["System.Description"];
  //   Get card mappings from database
  const { data, error } = await supabase
    .from("card-mapping")
    .select(
      "id, miroAppCardId::text, azurePBIId, miroUserId::text, azureUsername, created_at, miroBoardId, azurePBINumber, auth ( access_token )"
    )
